<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Forgot Password - Mazer Admin Dashboard</title>
    <link rel="shortcut icon"
        href="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20viewBox='0%200%2033%2034'%20fill-rule='evenodd'%20stroke-linejoin='round'%20stroke-miterlimit='2'%20xmlns:v='https://vecta.io/nano'%3e%3cpath%20d='M3%2027.472c0%204.409%206.18%205.552%2013.5%205.552%207.281%200%2013.5-1.103%2013.5-5.513s-6.179-5.552-13.5-5.552c-7.281%200-13.5%201.103-13.5%205.513z'%20fill='%23435ebe'%20fill-rule='nonzero'/%3e%3ccircle%20cx='16.5'%20cy='8.8'%20r='8.8'%20fill='%2341bbdd'/%3e%3c/svg%3e"
        type="image/x-icon">
    <link rel="shortcut icon"
        href="data:image/png;base64,"
        type="image/png">
    <link rel="stylesheet" crossorigin href="assets/compiled/css/app.css">
    <link rel="stylesheet" crossorigin href="assets/compiled/css/app-dark.css">
    <link rel="stylesheet" crossorigin href="assets/compiled/css/auth.css">
    <style>
        .input-depan {
            height: 90px;
            font-size: 50px;
            text-align: center;
        }

        .input-depan-dokter {
            height: 90px;
            font-size: 30px;
            text-align: center;
        }
    </style>
</head>

<body>
    <script src="assets/static/js/initTheme.js"></script>
    <div id="auth">

        <div class="row h-100">
            <div class="col-lg-5 col-12">
                <div id="auth-left">
                    <div class="auth-logo text-center">
                        <h2>SIANTRON</h2>
                    </div>
                    <h4>RUMAH SUNAT AMANAH</h4>
                    <p class="mb-4">Sistem Informasi Antrian Online</p>

                    <form action="/">
                        <label for="no_rkm_medis" class="form-label text-dark">Nomor Rekam Medis</label>
                        <div class="form-group position-relative mb-4">
                            <input type="text" class="form-control input-depan" name="no_rkm_medis" id="no_rkm_medis"
                                maxlength="8" placeholder="...">

                        </div>
                        <label for="id_dokter" class="form-label text-dark">Dokter</label>
                        <div class="form-group position-relative mb-4">
                            <select name="id_dokter" id="id_dokter" class="form-select input-depan-dokter">
                                <option value=""></option>
                                <option value="dr.">dr.Richo Wardana</option>
                            </select>

                        </div>
                        <button type="submit" class="btn btn-primary btn-block btn-lg shadow-lg">Lanjut</button>
                    </form>
                    <div class="text-center mt-3 text-lg fs-4">
                        <button type="submit" class="btn btn-success btn-block btn-lg shadow-lg mb-3">Gunakan
                            NIK</button>
                        <p class='text-gray-600'>Belum terdaftar?
                            <a href="" class="font-bold">Daftar Pasien Online</a>.
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-lg-7 d-none d-lg-block">
                <div id="auth-right">

                </div>
            </div>
        </div>

    </div>
</body>

</html>
